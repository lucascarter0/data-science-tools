...
plotting.set_labels(ax=ax, title='Motion of a Maneuvering Target',
                   xlabel='X', ylabel='Y')
ax.legend()
fig.tight_layout()
...
plotting.set_labels(ax=ax, title='CV and CA performance at Tracking Motion in X',
                   xlabel='Time (seconds)', ylabel='X Position')
ax.legend()
fig.tight_layout()
...
plotting.plot_r2((xs_cv - xs2)[t], dt=dt, ax=ax1)
plotting.plot_r2((xs_ca - xs2)[t], dt=dt, ax=ax1)

t = np.arange(len(xs2))[121:170]
plotting.plot_r2((xs_cv - xs2)[t], dt=dt, ax=ax2, t=t*dt)
plotting.plot_r2((xs_ca - xs2)[t], dt=dt, ax=ax2, t=t*dt)
...
plotting.plot_r2((xs_cv - xs2)[t], dt=dt, ax=ax1)
plotting.plot_r2((xs_ca - xs2)[t], dt=dt, ax=ax1)
plotting.plot_r2((xs - xs2)[t], dt=dt, ax=ax1)
ax1.legend(['CV Only', 'CA Only', 'MMAE'])

plotting.plot_probs(np.asarray(probs)[t],dt=dt, ax=ax2)
plotting.set_labels(ax=ax2, title='Probability weights in MMAE Filter',
                   xlabel='Time (seconds)', ylabel='$P$')
...
plotting.plot_r2((xs_imm - xs2)[t], dt=dt, ax=ax1)
ax1.legend(['IMM Filter'])

plotting.plot_probs(np.asarray(imm_prob)[t],dt=dt, ax=ax2)
ax2.legend(['$P(CV)$' ,'$P(CA)$'])
plotting.set_labels(ax=ax2, title='Probability weights in IMM Filter',
                   xlabel='Time (seconds)', ylabel='$P$')
fig.tight_layout()
...
plotting.plot_r2((xs - xs2)[t], dt=dt, ax=ax)
plotting.plot_r2((xs_imm - xs2)[t], dt=dt, ax=ax)
